...
-- ── Retroactively award achievements for existing players ──
-- This runs once to fix achievements for players who played before the system was working.
...
BEGIN
  FOR r IN SELECT id FROM public.players LOOP
    SELECT COUNT(*) INTO v_total_games FROM game_sessions WHERE player_id = r.id;
    SELECT COUNT(DISTINCT game_type) INTO v_distinct_types FROM game_sessions WHERE player_id = r.id;
    SELECT COUNT(*) INTO v_daily_count FROM daily_scores WHERE player_id = r.id;
    SELECT COALESCE(MAX(score), 0) INTO v_best_score FROM game_sessions WHERE player_id = r.id;

    -- Milestone achievements
    IF v_total_games >= 1 THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'first_win') ON CONFLICT DO NOTHING;
    END IF;
    IF v_total_games >= 5 THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'games_5') ON CONFLICT DO NOTHING;
    END IF;
    IF v_total_games >= 25 THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'games_25') ON CONFLICT DO NOTHING;
    END IF;
    IF v_total_games >= 50 THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'games_50') ON CONFLICT DO NOTHING;
    END IF;
    IF v_total_games >= 100 THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'games_100') ON CONFLICT DO NOTHING;
    END IF;

    -- Score achievements
    IF v_best_score >= 50 THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'score_50') ON CONFLICT DO NOTHING;
    END IF;
    IF v_best_score >= 100 THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'score_100') ON CONFLICT DO NOTHING;
    END IF;

    -- Difficulty achievements
    IF EXISTS (SELECT 1 FROM game_sessions WHERE player_id = r.id AND difficulty = 'medium') THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'medium_unlock') ON CONFLICT DO NOTHING;
    END IF;
    IF EXISTS (SELECT 1 FROM game_sessions WHERE player_id = r.id AND difficulty = 'hard') THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'hard_unlock') ON CONFLICT DO NOTHING;
    END IF;

    -- Exploration
    IF v_distinct_types >= 6 THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'all_games') ON CONFLICT DO NOTHING;
    END IF;

    -- Daily achievements
    IF v_daily_count >= 1 THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'daily_first') ON CONFLICT DO NOTHING;
    END IF;
    IF v_daily_count >= 10 THEN
      INSERT INTO player_achievements (player_id, achievement_id) VALUES (r.id, 'daily_10') ON CONFLICT DO NOTHING;
    END IF;
  END LOOP;
END $$;
